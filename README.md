...
The current release candidate is `v0.1.0-experimental.26`. It remains experimental pending validation on a physical Steam Deck. The v1 contract uses the same Wine prefix plus the explicit container re-entry path implemented by UniFiDeck's bundled UMU 1.4.4. This build also requires UMU's exact re-entry confirmation before reporting the trainer as running. Known trainer hashes expose finite, command-only controls through Trainer Relay; Epic and GOG trainers retain the direct sidecar launch path so a broken Wine layered-window surface cannot replace the game with an unusable virtual desktop.
...
2. Download the locally produced **`TrainerRelay.zip`** artifact. A GitHub release/tag for `v0.1.0-experimental.26` will be published only after the physical Steam Deck gate passes.
