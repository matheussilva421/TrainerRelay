...
The current release candidate is `v0.1.0-experimental.25`. It remains experimental pending validation on a physical Steam Deck. The v1 contract uses the same Wine prefix plus the explicit container re-entry path implemented by UniFiDeck's bundled UMU 1.4.4. This build also requires UMU's exact re-entry confirmation before reporting the trainer as running. Epic FLiNG trainers run inside a bounded Wine virtual desktop to avoid the layered-window black-screen path; the proven GOG launch path is unchanged.
...
2. Download the locally produced **`TrainerRelay.zip`** artifact. A GitHub release/tag for `v0.1.0-experimental.25` will be published only after the physical Steam Deck gate passes.
