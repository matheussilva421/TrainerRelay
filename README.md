...
This repository publishes `v0.1.0-experimental.10`. It is an experimental release pending validation on a physical Steam Deck. The v1 contract is the same Wine prefix, not a formal guarantee that the trainer runs inside the same pressure-vessel container as the game.
...
2. Download **`TrainerRelay.zip`** from the [experimental release](https://github.com/matheussilva421/TrainerRelay/releases/tag/v0.1.0-experimental.10).
...
- **Controls receive focus but pressing A does nothing:** install `v0.1.0-experimental.10`. `.9` matched CheatDeck's routed focus architecture, but a blocked or ready legacy migration still passed `disabled=true` to every configuration control. `.10` permits safe manual browsing and prefix editing while keeping relay enablement fail-closed until legacy launch options are repaired. Keep the CEF Console on **Default levels** and filter `[TrainerRelay:picker]` for validation.
